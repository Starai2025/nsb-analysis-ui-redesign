@echo off
echo ============================================
echo Pushing to GitHub Repository (FIXED)
echo ============================================
echo.
...
echo.
echo Fetching from GitHub first...
git fetch origin

echo.
echo Merging with remote (allowing unrelated histories)...
git branch -M main
git merge origin/main --allow-unrelated-histories -m "Merge initial GitHub commit" --no-edit 2>nul
...
if errorlevel 1 (
    echo.
    echo ============================================
    echo ALTERNATIVE: FORCE PUSH
    echo ============================================
    echo.
    echo If the above didn't work, we can force push
    echo (this will overwrite anything on GitHub)
    echo.
    set /p FORCE="Do you want to force push? (yes/no): "
    if /i "%FORCE%"=="yes" (
        echo.
        echo Force pushing...
