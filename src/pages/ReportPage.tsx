...
export default function ReportPage() {
  ...
  const generateReport = async () => {
    ...
    setStatusLine('Analyzing contract and correspondence...');
    ...
    try {
      ...
      setStatus('failed');
      setStatusLine('An error occurred during report generation.');
    }
  };
  ...
  const exportPDF = async () => {
    ...
    pdf.save(`NeverSignBlind_Report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
    ...
    <div className="max-w-6xl mx-auto py-12 px-12 space-y-12">
      <div className="flex justify-between items-start">
        <div className="space-y-2">
          ...
          <p className="text-on-surface-variant font-medium">Reference: CN-2024-0892 | Change Order Analysis</p>
        </div>

        <div className="flex items-center gap-3">
          <button 
            onClick={exportPDF}
            className="flex items-center gap-2 bg-white border border-slate-200 px-5 py-2.5 rounded shadow-sm hover:shadow-md transition-all active:scale-95 text-on-surface"
          >
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded shadow-lg transition-all active:scale-95 font-headline font-bold text-xs uppercase tracking-widest ${
              status === 'generating' 
      ...
      {status === 'generating' && (
        <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 text-center animate-pulse">
          <p className="text-sm font-bold text-primary">{statusLine}</p>
        </div>
      )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2 border-t border-slate-100">
                      <div className="flex justify-between text-[10px] text-slate-400 uppercase font-bold">
                        ...
                        <span>{new Date(analysis.noticeDeadline).toLocaleDateString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
